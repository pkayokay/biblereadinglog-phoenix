defmodule BiblereadinglogWeb.UserAuthTest do
  use BiblereadinglogWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Biblereadinglog.Accounts
  alias Biblereadinglog.Accounts.Scope
  alias BiblereadinglogWeb.UserAuth

  import Biblereadinglog.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, BiblereadinglogWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      BiblereadinglogWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        BiblereadinglogWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
